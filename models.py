...
class openAIActorWithStopping(Actor):
        def __init__(self, observationSpaceType, observationSpaceSize, actionSpaceType, actionSpaceSize, hiddenEncoderSize, maximumNumberOfHotBits, hiddenLayerParameters, actorCriticDevice = 'cpu'):
            super().__init__()
            self.observationSpaceType = observationSpaceType
            self.observationSpaceSize = observationSpaceSize
            ...
            self.numberOfHotBitsModel = explicitMLP(self.hiddenEncoderSize + 2, self.maximumNumberOfHotBits, self.defaultHiddenLayerSizes)
            ###
            #2 here stand for no or yes, so the categorical distribution is over no or yes.
            self.noActionModel = explicitMLP(self.hiddenEncoderSize + 3, 2, self.defaultHiddenLayerSizes)
            ###
            self.kHotVectorGenerator = explicitMLP(self.circulantSize, self.circulantSize, self.defaultHiddenLayerSizes)
            
            self.encoder2 = explicitMLP(self.hiddenEncoderSize + 4, self.circulantSize, self.defaultHiddenLayerSizes)
            self.to(actorCriticDevice)
        
        def actorActionToEnvAction(self, actorAction):
            i, j, k, hotCoordinates, noAction = actorAction
            """
            The actor is expected to produce i, j, and up to k coordinates which will be hot.
            The environment is expecting i,j and a binary vector, and an invalid bit
            """
            binaryVector = np.zeros(self.circulantSize, dtype = MODELS_BOOLEAN_TYPE)
            binaryVector[hotCoordinates[0:k]] = 1
            environmentStyleAction = [i, j, binaryVector, noAction]
            return environmentStyleAction
        ...
        def step(self, observations, action = None):
            ...
            if action is not None:
                ...
                # OSS 21/06/2022 notice that the following line was uncommented. Possibly a bug ?
                #k = kCategoricalDistribution.sample()
                #Omer Sella: k can't be 0
                k = kCategoricalDistribution.sample() + 1
            else:
            ...
            kTensor = k.unsqueeze(-1)
            kAppendedObservations = torch.cat([jAppendedObservations, kTensor], dim = -1)
            logitsForNoAction = self.noActionModel(kAppendedObservations)
            noActionCategoricalDistribution = Categorical(logits = logitsForNoAction)
            noActionDistributionEntropy = noActionCategoricalDistribution.entropy().unsqueeze(-1)
            ...
            if action is not None:
                ...
                noAction = torch.argmax(logitsForNoAction)
            

            noAction = noAction.float()
            noActionTensor = noAction.unsqueeze(-1)
            noActionAppendedObservations = torch.cat([kAppendedObservations, noActionTensor], dim = -1)            
            setEncodedStuff = self.encoder2(noActionAppendedObservations)
            # In this part we choose k coordinates, where: k <= maximumNumberOfHotBits <= circulantSize 
            # In practice we choose maximumNumberOfHotBits coordinates, and use only the first k of them
            if action is not None:
                coordinates = action[:, 3 : 3 + self.maximumNumberOfHotBits]
            ...
            #Omer Sella: remember that you added 1 to k so to get the log prob reduce 1
            logpK = kCategoricalDistribution.log_prob(k-1).unsqueeze(-1)#.sum(axis = -1)
            logpNoAction = noActionCategoricalDistribution.log_prob(noAction).unsqueeze(-1)
            
            if action is None:
                i = np.int32(i.item())
                j = np.int32(j.item())
                k = np.int32(k.item())
                noAction = np.int32(noAction.item())
                coordinates = np.int32(coordinates)
...
class openAIActorCritic(nn.Module):
    ...
    def __init__(self, observationSpaceType, observationSpaceSize, actionSpaceType, actionSpaceSize, hiddenEncoderSize, maximumNumberOfHotBits, hiddenLayerParameters, actorCriticDevice = 'cpu', actionInvalidator = 'Enabled'):
    
        super().__init__()
        self.actionInvalidator = actionInvalidator
        if self.actionInvalidator == 'Enabled':
            self.pi = openAIActorWithStopping(observationSpaceType, observationSpaceSize, actionSpaceType, actionSpaceSize, hiddenEncoderSize, maximumNumberOfHotBits, hiddenLayerParameters, actorCriticDevice)
        else:
    ...
    def step(self, obs, actions = None):
        ...
        if self.actionInvalidator == 'Enabled':
            logp_list = [logpI,  logpJ, logpK, logProbCoordinates, logpNoAction]
        else:
        ...
        if self.actionInvalidator == 'Enabled':
            entropy_list = [iDistributionEntropy, jDistributionEntropy, kDistributionEntropy, coordinateEntropies, noActionDistributionEntropy]
        else:
